...
module.exports = window.L.LayerGroup.extend({
    ...
    template: function(p) {
        p.status = 'open'; // hardcode for now
...
*/
    ...
    _load: function(map) {
        $.getJSON(this.API , {}, 
            // + '?bbox=' + this.boundsString(this._map),
            L.bind(function(resp) {
                this.notesLayer.eachLayer(L.bind(function(l) {
                    this.notesLayer.removeLayer(l);
                }, this));
                window.resp = resp; // global variable for console debugging
                resp.features = resp.results; // GeoJSON standard name
                for (var i = 0; i < resp.features.length; i++) {
                    feature = resp.features[i];
                    feature.geometry = feature.geom; // GeoJSON
                    feature.properties = this.template(feature.layers['test.fillarikanava2011_test'].data); // GeoJSON
